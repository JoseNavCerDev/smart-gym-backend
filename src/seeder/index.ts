...
const getUserData = async (id_role: Types.ObjectId):Promise<UserType | undefined> => {
  ...
  if(!salt || !name || !user || !password || !email || !status) return undefined
...
const seedAll = async () => {
  try {
    ...
    if (roles.length === 0) roles = await Role.insertMany(rolesData)
    ...
    if (users.length === 0) {
      ...
      if (!USER_DATA) throw new Error('Enviroment data is missing [seeder]')
